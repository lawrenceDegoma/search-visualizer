...
app.get("/search", (req, res) => {
  ...
  const start = performance.now(); // measure start

  const results = data
    .map((doc) => {
      const docStart = performance.now();
      const score = getScore(query, doc);
      const docEnd = performance.now();

      return {
        ...doc,
        score,
        processingTime: docEnd - docStart, // ms per doc
      };
    })
  ...
  const end = performance.now(); // measure end

  console.log({
    query,
    totalDocs: data.length,
    returned: results.length,
    totalTimeMs: (end - start).toFixed(3),
    avgDocTimeMs: (end - start) / data.length,
  });
  ...
  res.json({
    ...
    metrics: {
      ...
      avgDocTimeMs: Number(((end - start) / data.length).toFixed(6)),
      returned: results.length,
    },
  });
});
